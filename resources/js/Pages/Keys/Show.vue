<template>
    <AppLayout v-if="keyData" :title="keyData.game?.name || 'Key'">
        <Head :title="`${keyData.game?.name || 'Key'} - ${keyData.platform?.name || 'Key'}`" />
        <div class="max-w-7xl mx-auto px-4 py-6">
            <!-- Flash Messages -->
            <div v-if="flash.error" class="mb-6 bg-red-900/50 border border-red-700 rounded-lg p-4 text-red-300">
                {{ flash.error }}
            </div>
            <div v-if="flash.message" class="mb-6 bg-green-900/50 border border-green-700 rounded-lg p-4 text-green-300">
                {{ flash.message }}
            </div>

            <!-- Game Information Section -->
            <div v-if="keyData.game" class="mb-8">
                <div class="bg-dark-800 rounded-lg border border-dark-700 p-6">
                    <div class="flex flex-col lg:flex-row gap-6">
                        ...
                        <div class="flex-1">
                            ...
                            <div v-if="keyData.platform?.name" class="inline-flex items-center gap-2 px-4 py-2 bg-primary-600/20 border border-primary-600/30 rounded-lg">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
                ...
                <div class="lg:col-span-2 bg-dark-800 rounded-lg border border-dark-700 p-6">
                    <div class="flex items-center justify-between mb-4">
                        <h2 class="text-xl font-bold text-white">Key Details</h2>
                        <div v-if="keyData.platform?.name" class="inline-flex items-center gap-2 px-3 py-1.5 bg-primary-600/20 border border-primary-600/30 rounded-lg">
                    ...
                    <form @submit.prevent="claimKey" class="space-y-4">
                        ...
                        <div v-else class="space-y-4">
                            ...
                            <p class="text-gray-400 text-sm text-center">This key has already been claimed</p>
                        </div>
                    </form>
                </div>
                ...
                <!-- Shared By Section -->
                <div class="bg-dark-800 rounded-lg border border-dark-700 p-6">
                    <h2 class="text-xl font-bold text-white mb-4">Shared By</h2>
                    <div class="flex flex-col items-center text-center">
                        ...
                        <p v-if="keyData.createdUser?.bio" class="text-gray-400 text-sm">{{ keyData.createdUser.bio }}</p>
                    </div>
                </div>
            </div>

            <!-- Message Section -->
            <div v-if="keyData.message" class="bg-dark-800 rounded-lg border border-dark-700 p-6">
                <h2 class="text-xl font-bold text-white mb-3">Message from Sharer</h2>
                <p class="text-gray-300 leading-relaxed">{{ keyData.message }}</p>
            </div>
        </div>
    </AppLayout>
        ...
        <div class="max-w-7xl mx-auto px-4 py-6">
            <div class="bg-dark-800 rounded-lg border border-dark-700 p-12 text-center">
                <div class="inline-flex flex-col items-center space-y-4">
                    <div class="w-16 h-16 border-4 border-accent-600 border-t-transparent rounded-full animate-spin"></div>
